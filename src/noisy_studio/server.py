...
LISTENER_PORT_ENV_VAR = "NOISY_STUDIO_LISTENER_PORT"
...
async def _identity_error(agent_id: str | None) -> str | None:
    ...
    if agent_id and agent_id.strip():
        return None
    message = (
        "Voice session identity is missing: the noisy-studio PreToolUse hook did not run "
...
def _ensure_daemon() -> None:
    ...
    place without touching the server. Set NOISY_STUDIO_NO_AUTOSPAWN=1 to opt
    out (e.g. to always manage the daemon by hand).
    """
    if os.environ.get("NOISY_STUDIO_NO_AUTOSPAWN"):
        return
    ...
    try:
        ...
        subprocess.Popen(
            [sys.executable, "-m", "noisy_studio.listener.daemon"],
...
def main() -> None:
    ...
    if os.environ.get("NOISY_STUDIO_MCP_TRANSPORT", "stdio") == "http":
        mcp.settings.host = os.environ.get("NOISY_STUDIO_MCP_BIND", "127.0.0.1")
        mcp.settings.port = int(os.environ.get("NOISY_STUDIO_MCP_PORT", "8767"))
