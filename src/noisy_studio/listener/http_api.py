...
from noisy_studio.providers.config import ConfigurationError, recovery_info
from noisy_studio.listener import stt_lab
from noisy_studio.listener import pricing, speech
from noisy_studio.listener.dashboard import DASHBOARD_HTML
...
try:
    ...
    DAEMON_VERSION = display_version(_pkg_version("noisy-studio"))
...
LATEST_VERSION_URL = (
    "https://raw.githubusercontent.com/noisy/noisy-studio/main/.claude-plugin/plugin.json"
)
...
from noisy_studio.listener.state import VOICE_POOL as SUBAGENT_VOICE_POOL  # noqa: E402
from noisy_studio.listener.state import hash_pick as _hash_pick  # noqa: E402
...
BIND_ENV_VAR = "NOISY_STUDIO_BIND"
...
PORT_ENV_VAR = "NOISY_STUDIO_LISTENER_PORT"
...
def _apply_hotkeys(state: ListenerState, patch: dict[str, str]) -> tuple[dict, dict]:
    ...
    from noisy_studio.listener.chords import problems
...
def status_payload(state: ListenerState) -> dict:
    ...
    from noisy_studio import providers as _providers
    from noisy_studio.providers import selection
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _handle_GET(self) -> None:
            ...
            if url.path == "/":
                ...
                from noisy_studio.providers import config as provider_config
                from noisy_studio.providers import local as local_provider
        ...
        def _handle_POST(self) -> None:
            if self.path == "/delivery/acknowledge":
                ...
                import wave
                from noisy_studio.providers import selection
                from noisy_studio.providers import stt_provider
                body = self._read_json_body()
                try:
                    candidate = selection.choice(str(body.get('choice', '')))
                ...
                import base64
                from noisy_studio.providers import selection
                from noisy_studio.providers import tts_provider
                body = self._read_json_body()
                try:
                    candidate = selection.choice(str(body.get('choice', '')))
