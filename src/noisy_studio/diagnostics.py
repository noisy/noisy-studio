...
from noisy_studio.listener import stt, stt_stream
...
async def _check_billing() -> None:
    ...
    url = BILLING_URL_TEMPLATE.format(team_id=os.environ.get(TEAM_ID_ENV_VAR))
    headers = {"Authorization": f"Bearer {os.environ.get(MANAGEMENT_KEY_ENV_VAR)}"}
    async with httpx.AsyncClient(timeout=CHECK_TIMEOUT_SECONDS) as client:
