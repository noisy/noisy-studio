...
export const ProviderAvailability:StoryObj={
  render:()=>({setup(){
    ...
    <div class="availability-row"><div><h3>End silence</h3><p>{{turn==='ptt' ? 'Applies only when Turn detection is Auto.' : 'Pause before ending an Auto turn.'}}</p></div><select aria-label="End silence" v-model="silence"><option v-for="ms in silenceOptions" :key="ms" :value="ms">{{ms/1000}}s</option></select></div>
