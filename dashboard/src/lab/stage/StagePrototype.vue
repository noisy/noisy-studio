...
export interface StageLine { id: number; person: string; text: string }
const props = withDefaults(defineProps<{
  people: StagePerson[]; speaking?: string; lines?: StageLine[];
  initialDisplay?: 'heads' | 'caption' | 'thread'; reducedMotion?: boolean;
}>(), { speaking: '', lines: () => [], initialDisplay: 'heads', reducedMotion: false });
...
const currentLine = computed(() => [...props.lines].reverse().find(line => line.person === props.speaking));
...
<template>
  <section class="stage" :class="{ 'reduce-motion': reducedMotion, 'has-thread': display === 'thread' }" aria-label="Stage preview">
    <header class="stage-header">
      ...
      <button class="stage-exit" @click="emit('exit')">↙ Back to dashboard <kbd>Esc</kbd></button>
    </header>
    <div class="stage-body">
      <div class="constellation" :class="{ dense, solo: people.length === 1 }" aria-label="Conversation portraits">
        ...
        <article v-for="person in visible" :key="person.id" class="person" :class="{ speaking: speaking === person.id, focused: focus === person.id, offline: person.offline, muted: person.muted }" :style="position(person.id)" :data-person="person.id">
          <button class="person-focus" :aria-label="`Focus on ${person.name}`" :aria-pressed="focus === person.id" @click="focus = focus === person.id ? '' : person.id">
            <span class="portrait-shell"><VoiceAvatar :voice="person.voice" :size="126" set="editorial" /><span v-if="speaking === person.id" class="speaking-bars" aria-hidden="true"><i/><i/><i/><i/></span></span>
      ...
      <aside v-if="display === 'thread'" class="stage-thread" aria-label="Shared transcript preview"><div class="thread-heading"><span>THE CONVERSATION</span><span>Preview</span></div><p class="thread-note">A shared view. Each agent keeps its own conversation.</p><div v-for="line in thread" :key="line.id" class="thread-line"><strong>{{ people.find(p => p.id === line.person)?.name ?? 'You' }}</strong><p>{{ line.text }}</p></div><p v-if="!thread.length" class="thread-note">Words will appear here.</p></aside>
    </div>
    <div v-if="display === 'caption'" class="stage-caption" aria-live="polite"><span>{{ speaker?.name ?? 'CURRENT CAPTION' }}</span><p>{{ currentLine?.text ?? 'When someone speaks, their words appear here.' }}</p></div>
