...
export const AUDIO_CONTROL_IDS = ['microphone', 'language', 'turn', 'agent', 'recognition', 'cues', 'silence', 'sensitivity', 'smart'] as const;
...
export function audioControls(status: DaemonStatus | null, devices: InputDevice[], cues: boolean) {
  ...
  return [
    ...
    {id:'silence',label:'End silence',help:'Pause before ending an Auto turn. Zero closes at the first detected pause.',value:String(s?.end_silence_ms ?? 2000),options:END_SILENCE_MS.map(ms=>({value:String(ms),label:`${ms/1000}s`,disabled:false})),disabled:s?.detection_mode === 'ptt'},
