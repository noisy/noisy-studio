...
describe('speech selection',()=>{
  ...
  it('offers a route to setup rather than an unusable apply button alone',async()=>{api.getSpeechSettings.mockResolvedValue(speechFixture('setup'));const wrapper=await openVoices();await wrapper.findAll('button').find(b=>b.text()==='Open System settings')!.trigger('click');expect(wrapper.emitted('configure')).toEqual([[]]);wrapper.unmount();});
});
