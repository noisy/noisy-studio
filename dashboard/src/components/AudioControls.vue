...
<template>
  <div class="audio-controls" :class="{settings}" :style="{ '--audio-settings-max-width': SETTINGS_MAX_WIDTH + 'px' }">
    ...
    <div v-for="c in controls" :key="c.id" class="audio-row" :title="c.notice || (c.disabled ? 'Used in Auto mode' : c.help)">
      <div><label :id="`${prefix}-${c.id}-label`" :for="c.buttons ? undefined : `${prefix}-${c.id}`">{{c.label}}</label><p v-if="settings" :id="`${prefix}-${c.id}-help`" class="help" :class="{notice:!!c.notice}">{{c.notice || c.help}}</p></div>
      ...
      <select v-else :id="`${prefix}-${c.id}`" :value="c.value" :disabled="c.disabled" :aria-describedby="settings || c.id === 'silence' ? `${prefix}-${c.id}-help` : undefined" @focus="c.id === 'microphone' && emit('refreshDevices')" @change="emit('change',{id:c.id,value:($event.target as HTMLSelectElement).value})"><option v-for="o in c.options" :key="o.value" :value="o.value">{{o.label}}</option></select>
      ...
      <p v-if="!settings && c.id === 'silence'" :id="`${prefix}-${c.id}-help`" class="dashboard-notice notice">{{c.notice}}</p>
    </div>
