...
- [x] Verify packaged engine and actual dev UI/history/microphone/voice using postponed restart. Record release-only signed-upgrade and host-reconnection limits. Add PR #90 to local post-push follow-ups without publishing.
...
Validation: Python unit/harness suites, frontend and desktop checks, dashboard/Storybook builds, canonical wheel metadata/entry points, and frozen MCP/hook smoke passed. The renamed dev daemon restarted against the explicitly selected existing store; saved conversation count, Jabra selection, state WebSocket and UI proxy were verified. Host Claude dev hooks/MCP settings were subsequently updated only after explicit user authorization, with private backups and configured-launcher checks.

Remaining release checks: reload the affected agent sessions and verify incoming voice with their renamed tools; test a signed desktop upgrade and its OS permissions. The current dev launch deliberately retains its existing store via NOISY_STUDIO_DEV_CONFIG_DIR; use that override on subsequent launches until explicitly migrating the store. No push or public PR update was made.
