...
`compare_cloud.py` defaults to a plan-only run: it does not load credentials or call providers without `--allow-cloud`. After explicit authorization, that flag sends exactly the five named hero-v1 clips to xAI, one request per clip, and compares them with cached Whisper base while alternating execution order. Existing source recordings and live engine selections are unchanged. The runner records exception types without provider response bodies and does not retry failed requests.

Run `PYTHONPATH=src python tools/voice-evaluation/compare_cloud.py` to inspect the plan. Add `--allow-cloud` only when the owner has approved transmitting those demo recordings and normal API charges. Actual results will be saved as `cloud-results.json`; this runner has only been syntax-checked and run in plan-only mode so far. Five samples are not a stable median/p95 latency distribution and do not verify streaming, endpointing, Polish or subjective voice quality.
